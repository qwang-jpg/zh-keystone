...
// (object-contain, no cropping, no stretching).
export default function EvidenceCard({ src, label, compact = false, onClick, className }) {
  ...
  return (
    <figure
      className={cn(
        "group flex shrink-0 flex-col overflow-hidden rounded-2xl border border-keystone-ink/10 bg-white shadow-sm",
        compact ? "h-28 sm:h-36 md:h-48" : "h-56 sm:h-72 md:h-96",
        className
      )}
    >
      <button
      ...
      >
        <img
          ...
          className="h-full w-auto max-w-[calc(100vw-3rem)] rounded-lg object-contain transition-transform duration-300 group-hover:scale-[1.02]"
        />
        {clickable && (
